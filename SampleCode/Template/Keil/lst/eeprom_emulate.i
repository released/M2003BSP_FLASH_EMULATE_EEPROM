...
static uint8_t Written_Data[128UL];
...
void FMC_Enable(void)
{
  ...
  ((CLK_T *) (((uint32_t)0x40000000UL) + 0x00200UL))->AHBCLK |= (0x1ul << (1));
...
uint32_t Init_EEPROM(uint32_t data_amount, uint32_t use_pages)
{
  ...
  if(Amount_of_Data > 128UL)
    return 0x01;

  if((Amount_Pages == 0UL) || (Amount_Pages > 8UL))
    return 0x02;
...
void Search_Valid_Page(void)
{
  ...
  {
    Page_Status[i] = (uint16_t)FMC_Read(((0x7800UL)) + ((0x200) * i));
  }


  for(i = 0; i < Amount_Pages; i++)
  {
    if(Page_Status[i] != 0xFFFF)
      Current_Valid_Page = i;
  }

  if(Page_Status[Current_Valid_Page] == 0xFFFF)
  {

    FMC_Write(((0x7800UL)) + ((0x200) * Current_Valid_Page), 0xFFFF0000);
  ...
  else
  {
    ...
    temp = FMC_Read(((0x7800UL)) + ((0x200) * Current_Valid_Page));
    addr = (temp & 0xFF0000) >> 16UL;
    data = (temp & 0xFF000000) >> 24UL;
    ...
    else
    {
      ...
      if(addr < Amount_of_Data)
      {
        Written_Data[addr] = data;
      }


      Current_Cursor = (0x200);


      for(i = 4; i < (0x200); i += 4)
      {

        temp = FMC_Read(((0x7800UL)) + ((0x200) * Current_Valid_Page) + i);
        addr = (temp & 0xFF) >> 0UL;
        data = (temp & 0xFF00) >> 8UL;
...
uint32_t Write_Data(uint8_t index, uint8_t data)
{
  ...
  if((Current_Cursor & 0x3) == 0)
  {
    ...
    temp = 0xFFFF0000 | (index << 0UL) | (data << 8UL);
    FMC_Write(((0x7800UL)) + ((0x200) * Current_Valid_Page) + Current_Cursor, temp);
  ...
  else
  {
    ...
    temp = FMC_Read(((0x7800UL)) + ((0x200) * Current_Valid_Page) + (Current_Cursor - 2));

    temp &= ~(0xFF0000 | 0xFF000000);
    temp |= (index << 16UL) | (data << 24UL);

    FMC_Write(((0x7800UL)) + ((0x200) * Current_Valid_Page) + (Current_Cursor - 2), temp);

    Written_Data[index] = data;
  }


  if(Current_Cursor == ((0x200) - 2))
  {
...
void Manage_Next_Page(void)
{
  ...
  counter = FMC_Read(((0x7800UL)) + ((0x200) * Current_Valid_Page));
  ...
  while((i < Amount_of_Data) && (Written_Data[i] == 0xFF))
  {
    i++;
  }
  ...
  if(i >= Amount_of_Data)
  {
    ...
    counter = (counter & 0xFFFFUL) | 0xFFFF0000UL;
    FMC_Write(((0x7800UL)) + ((0x200) * new_page), counter);
    Current_Cursor = 2;
    FMC_Erase(((0x7800UL)) + ((0x200) * Current_Valid_Page));
    Current_Valid_Page = new_page;
    return;
  }


  counter &= ~(0xFF0000 | 0xFF000000);
  counter |= (i << 16UL) | (Written_Data[i] << 24UL);
  FMC_Write(((0x7800UL)) + ((0x200) * new_page), counter);
  i++;
  ...
  for(j = 4; i < Amount_of_Data; i++)
  {
    ...
    else
    {
      ...
      if(data_flag == 0)
      {
        temp |= (i << 0UL) | (Written_Data[i] << 8UL);
        data_flag = 1;
      }

      else
      {
        temp |= (i << 16UL) | (Written_Data[i] << 24UL);
        FMC_Write(((0x7800UL)) + ((0x200) * new_page) + j, temp);
        temp = 0;
        data_flag = 0;
        j += 4;
      }
...
uint16_t Get_Cycle_Counter(void)
{
  ...
  Cycle_Counter = (uint16_t)FMC_Read(((0x7800UL)) + ((0x200) * Current_Valid_Page));
