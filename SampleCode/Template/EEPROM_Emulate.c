...
static uint8_t	Written_Data[Max_Amount_of_Data];
...
uint32_t Init_EEPROM(uint32_t data_amount, uint32_t use_pages)
{
		...
		/* Current implementation supports up to 8 emulation pages. */
		if((Amount_Pages == 0UL) || (Amount_Pages > Max_Amount_of_Page))
				return	Err_OverPageAmount;		

		/* Fill initial data 0xFF*/
		for(i = 0; i < Max_Amount_of_Data; i++)
		{
				Written_Data[i] = 0xFF;
		}
...
void Search_Valid_Page(void)
{
		uint32_t i, temp;
		uint8_t	addr, data;
		uint16_t	Page_Status[Max_Amount_of_Page];
		...
		{
				Page_Status[i] = (uint16_t)FMC_Read(DataFlash_BaseAddr + (FMC_FLASH_PAGE_SIZE * i));
		}
	
		/* Search which page has valid data */
		for(i = 0; i < Amount_Pages; i++)
		{
				if(Page_Status[i] != Status_Unwritten)
						Current_Valid_Page = i;
		}
		/* If Data Flash is used for first time, set counter = 0 */
		if(Page_Status[Current_Valid_Page] == Status_Unwritten)
		{
				/* Set counter = 0 */
		...
		else
		{
				...
				else
				{
						...
						/* Assume the page is full unless an unwritten slot is found. */
						Current_Cursor = FMC_FLASH_PAGE_SIZE;
...
uint32_t Write_Data(uint8_t index, uint8_t data)
{
		...
		/* Recover from a reboot when the previous page was already full. */
		if(Current_Cursor >= FMC_FLASH_PAGE_SIZE)
		{
				Manage_Next_Page();
		}
...
void Manage_Next_Page(void)
{
		...
		if(i >= Amount_of_Data)
		{
				/* No valid user data left, keep only cycle counter in the new page. */
