 ...
 ******************************************************************************/
#ifndef EEPROM_EMULATE_H
#define EEPROM_EMULATE_H

#include <stdio.h>
#include "NuMicro.h"

#define DATA_FLASH_OFFSET  								(0x7800UL)		/* 32 KB APROM, use the last 4 pages (4 x 512 bytes) */

#define DATA_FLASH_AMOUNT								(24)
#define DATA_FLASH_PAGE  								(4)


#define DataFlash_BaseAddr			                    (DATA_FLASH_OFFSET) /* (FMC->DFBA) / (FMC->DFBADR) */
#define Max_Amount_of_Data			                    128UL
#define Max_Amount_of_Page			                    8UL
